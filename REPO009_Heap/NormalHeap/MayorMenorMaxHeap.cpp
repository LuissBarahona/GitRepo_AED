...
public:
    ...
    void ordenarMayorMenor(){
        // Crear una copia del MaxHeap original
        ArrayToMaxHeap heapCopia;
        copiarHeap(raiz, heapCopia);

        // Crear una cola para almacenar los valores ordenados
        ColaGenerica<int> c;

        // Extraer el máximo elemento del heap hasta que esté vacío y encolarlo
        while (heapCopia.tamanoActual() > 0) {
            int maximo = heapCopia.obtenerMaximo();  // Obtener el máximo
            c.encolar(maximo);  // Encolar el máximo
            heapCopia.eliminar(maximo);  // Eliminar el máximo
        }

        // Imprimir los valores encolados
        std::cout << "Ordenar de mayor a menor: ";
        c.imprimir();
    }
     ...
     // Función auxiliar para copiar el heap original a otro heap
    void copiarHeap(Nodo* nodo, ArrayToMaxHeap& heapCopia) {
        if (nodo == nullptr) return;
        heapCopia.insertar(nodo->valor);
        copiarHeap(nodo->izquierdo, heapCopia);
        copiarHeap(nodo->derecho, heapCopia);
    }
