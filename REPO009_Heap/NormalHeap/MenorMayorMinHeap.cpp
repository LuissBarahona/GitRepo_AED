...
public:
    ...
    void ordenarMenorMayor() {
        std::cout << "Ordenar de menor a mayor: " << std::endl;

        // Crear una copia del heap original
        ArrayToMinHeap heapCopia;
        copiarHeap(raiz, heapCopia);

        // Crear una cola para almacenar los valores ordenados
        ColaGenerica<int> p;

        // Extraer el mínimo elemento del heap hasta que esté vacío y encolarlo
        while (heapCopia.tamanoActual() > 0) {
                int minValor = heapCopia.obtenerMinimo();
                p.encolar(minValor);
                heapCopia.eliminar(minValor);
        }
    ...
    void copiarHeap(Nodo* nodo, ArrayToMinHeap& heapCopia) {
        if (nodo == nullptr) return;
...
int main() {
    ...
    heap.inorder();
     // Probar la función de ordenar de menor a mayor
    heap.ordenarMenorMayor();
